...
partial class MainForm
{
    ...
    internal (string StatusText, Color BackColor) GetRequestItemVisualStatus(AssetRequestItem request, GenerationItemRecord? preloadedJob = null)
    {
        ...
        var job = preloadedJob ?? (_currentManifest != null ? _generationJobStore.GetItem(_currentManifest.ManifestFingerprint, request.RequestKey) : null);
        if (job != null)
        {
            if (job.Status == GenerationItemStatus.Ready)
            {
                ...
                return ("Candidate missing", FailedRowBackColor);
            }
